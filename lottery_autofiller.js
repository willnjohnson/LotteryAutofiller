...
(function () {
  ...
  function addResetButton() {
    ...
    if (contentTd) {
      ...
      resetBtn.addEventListener('mouseenter', () => {
        resetBtn.style.backgroundColor = '#003FBF';
      });

      resetBtn.addEventListener('mouseleave', () => {
        resetBtn.style.backgroundColor = '#007FFF';
      });
  ...
  function waitForInputs(callback, timeout = 2000) {
    const start = Date.now();
    const check = () => {
      const allFound = ['one', 'two', 'three', 'four', 'five', 'six'].every(name =>
        document.querySelector(`input[name="${name}"]`)
      );
      if (allFound) return callback();
      if (Date.now() - start < timeout) setTimeout(check, 50);
    };
    check();
  }

  // Main logic wrapped after page load + input wait
  waitForInputs(() => {
